                        ...
                        <div class="card">
                             <?php
                              ...
                              if(isset($_POST['add'])){
                                ...
                                $sql = "INSERT INTO messages_a_to_s (content , message_date , medication_time , academic_supervisor_id , type , opposer_id) VALUES ('$message' , '$chat_date' , '$chat_time' , '$acad_id' , 'A_To_S' , '$student_id')";
                            ...
                            include('../connect.php');  
                             ...
                             $stmtt = $con->prepare("SELECT name  FROM students WHERE id='$student_id'");
                             $stmtt->execute();
                             $rowss = $stmtt->fetch();

                             ?> 
                            <div class="card-body">
                                <h4 class="card-title mb-4">تفاصيل المحادثة مع <strong>الطالب /  <?php echo $rowss['name']; ?></strong></h4>
                                ...
                                <div class="chat-container">
                                    ...
                                    <?php
                                        ...
                                        $sql = $con->prepare("SELECT
                                          messages_a_to_s.* , academic_admins.id , students.name as student_name
                                       FROM
                                          messages_a_to_s
                                       ...
                                       ON
                                          messages_a_to_s.student_id = students.id");
                                        $sql->execute();
                                        $rows = $sql->fetchAll();
                                        ...
                                        foreach($rows as $pat)
                                        {
                                            ...
                                            if($pat["academic_supervisor_id"] == $rows1['id'] && $pat["opposer_id"] == $student_id){

                                      ?>
                                    <div class="chat-message bg-primary text-light py-5 px-3 rounded my-2">
                                        <strong>أنا:</strong> <?php echo $pat['content']; ?>
                                    </div>
                                    <?php }elseif($pat["student_id"] == $student_id && $pat["opposer_id"] == $rows1['id']){ ?>
                                    <div class="chat-message bg-light text-dark py-5 px-3 rounded my-2">
                                        <strong><?php echo $pat['student_name']; ?>:</strong> <?php echo $pat['content']; ?>
                                    </div>
                                    <?php }} ?>
