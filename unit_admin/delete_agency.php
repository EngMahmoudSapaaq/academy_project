                        ...
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item active">ادارة الجهات التدريبية</li>
                                </ol>
                            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <h4 class="card-title">كل الجهات التدريبية</h4>
                                    <div>
                                        <a href="training_agencies_create.php" class="btn btn-success"><i class="fa fa-plus"></i> اضافة جهة تدريبية</a>
                                    </div>
                                </div>
...
// التحقق من وجود سجلات طلاب مرتبطة بالجهة التدريبية
$checkStmt = $con->prepare("SELECT COUNT(*) FROM students WHERE training_agency_id = :agency_id");
$checkStmt->execute([':agency_id' => $agency_id]);

if ($checkStmt->fetchColumn() > 0) {
    // إذا كانت هناك سجلات مرتبطة، عرض رسالة منع الحذف
    echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
            <div class="alert alert-danger" role="alert" style="color:#000;text-align:center;margin-bottom:40px;font-family:cairo">
                لا يمكن حذف هذه الجهة لأنها مرتبطة بسجلات طلاب.
            </div>
          </div>';
} else {
    // إذا لم يكن هناك سجلات مرتبطة، متابعة الحذف
    $stmt = $con->prepare("DELETE FROM training_agencies WHERE id = :agency_id");
    ...
    echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
            <div class="alert alert-success" role="alert" style="color:#FFF;text-align:center;margin-bottom:40px;font-family:cairo">
                تم حذف هذه الجهة بنجاح
            </div>
          </div>';
}

// عرض قائمة الوكالات التدريبية
...
?>      
    <tr>
        ...
        <td><?php echo $pat['max_chances']; ?></td>
        <td>
            <a href="training_agencies_edit.php?agency_id=<?php echo $pat['id']; ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
            <a onclick="return confirm('هل أنت متأكد من عملية الحذف ؟');" href="delete_agency.php?agency_id=<?php echo $pat['id']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i></a>
        </td>
    </tr>
<?php } ?>    
