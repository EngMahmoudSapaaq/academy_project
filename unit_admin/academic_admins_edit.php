...
if(isset($_POST['edit'])){
    ...
    // التحقق من عدم تكرار البريد الإلكتروني أو الهاتف أو اسم المستخدم
    $sql = $con->prepare("SELECT * FROM academic_admins WHERE (email = ? OR phone = ?) AND id != ?");
    $sql->execute([$email, $phone, $acad_id]);
    $countEmailPhone = $sql->rowCount();

    $sql2 = $con->prepare("SELECT * FROM accounts WHERE username = ? AND id != ?");
    $sql2->execute([$username, $account_id]);
    $countUsername = $sql2->rowCount();

    if($countEmailPhone == 0 && $countUsername == 0){

        if(!empty($_POST['password']) && !empty($_POST['confirm'])){
            if($_POST['password'] == $_POST['confirm']){
                $stmt = $con->prepare("UPDATE academic_admins SET email = ?, phone = ? WHERE id = ?");
                $stmt->execute([$email, $phone, $acad_id]);
                ...
                echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
                          <div class="alert alert-success" role="alert" style="color:#FFF;text-align:center;margin-bottom:40px;font-family:cairo">
                              تم تحديث بيانات المشرف الأكاديمي بنجاح
                          </div>
                      </div>';
            } else {
                echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
...
?>
                                      ...
                                      <?php
                                        ...
                                        include('../connect.php');
                                        $stmt = $con->prepare("SELECT academic_admins.* , accounts.username FROM academic_admins INNER JOIN accounts ON accounts.id=academic_admins.account_id WHERE academic_admins.id='$acad_id'");  
                                        $stmt->execute();
                                        $rowsSpec = $stmt->fetch();
                                        $count = $stmt->rowCount();
                                      ...
                                      ?> 
                                        <form method="post" class="form">
                                            <input type="hidden" name="acad_id" value="<?php echo $rowsSpec['id']; ?>"/>
