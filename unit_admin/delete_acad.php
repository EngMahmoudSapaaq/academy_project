                        ...
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item active">ادارة المشرفين الأكاديميين</li>
                                </ol>
                            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <h4 class="card-title">كل المشرفين الأكاديميين</h4>
                                    <div>
                                        <a href="academic_admins_create.php" class="btn btn-success"><i class="fa fa-plus"></i> اضافة مشرف أكاديمي</a>
                                    </div>
                                </div>
                                <div class="table-responsive">
                                    <table class="table table-centered table-nowrap mb-0">
                                     ...
                                     <?php
include('../connect.php');  
$acad_id = isset($_GET['acad_id']) && is_numeric($_GET['acad_id']) ? intval($_GET['acad_id']) : 0;
$account_id = isset($_GET['account_id']) && is_numeric($_GET['account_id']) ? intval($_GET['account_id']) : 0;

// التحقق من وجود سجلات في جدول الشعب مرتبطة بالمشرف الأكاديمي
$checkStmt = $con->prepare("SELECT COUNT(*) FROM majors WHERE academic_admin_id = :acad_id");
$checkStmt->execute([':acad_id' => $acad_id]);

if ($checkStmt->fetchColumn() > 0) {
    // إذا كانت هناك سجلات مرتبطة، إظهار رسالة منع الحذف
    echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
            <div class="alert alert-danger" role="alert" style="color:#FFF;text-align:center;margin-bottom:40px;font-family:cairo">
                لا يمكن حذف هذا المشرف الأكاديمي لأنه مرتبط بسجل شعبة.
            </div>
          </div>';
} else {
    // إذا لم يكن هناك سجلات مرتبطة، متابعة الحذف
    $stmt = $con->prepare("DELETE FROM academic_admins WHERE id = :acad_id");
    $stmt->bindParam(":acad_id", $acad_id);
    $stmt->execute();
    
    $stmt1 = $con->prepare("DELETE FROM accounts WHERE id = :account_id");
    $stmt1->bindParam(":account_id", $account_id);
    $stmt1->execute();    

    echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
            <div class="alert alert-success" role="alert" style="color:#000;text-align:center;margin-bottom:40px;font-family:cairo">
                تم حذف هذا المشرف الأكاديمي بنجاح
            </div>
          </div>';
}

// عرض قائمة المشرفين الأكاديميين
$sql = $con->prepare("SELECT academic_admins.*, accounts.username FROM academic_admins INNER JOIN accounts ON accounts.id = academic_admins.account_id");
$sql->execute();
$rows = $sql->fetchAll();
...
?>     
    <tr>
        ...
        <?php } ?>
        <td>
            <a href="academic_admins_edit.php?acad_id=<?php echo $pat['id']; ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
            <a onclick="return confirm('هل أنت متأكد من عملية الحذف ؟');" href="delete_acad.php?acad_id=<?php echo $pat['id']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i></a>
        </td>
    </tr>
<?php } ?>    
