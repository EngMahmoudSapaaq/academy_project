                        ...
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item active">ادارة الشعب</li>
                                </ol>
                            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-center mb-2">
                                    <h4 class="card-title">كل الشعب</h4>
                                    <div>
                                        <a href="major_create.php" class="btn btn-success"><i class="fa fa-plus"></i> اضافة شعبة</a>
                                    </div>
                                </div>
...
$stmtCheck = $con->prepare("SELECT COUNT(*) FROM students WHERE major_id = :major_id");
$stmtCheck->bindParam(":major_id", $major_id);
$stmtCheck->execute();
$studentCount = $stmtCheck->fetchColumn();

if ($studentCount > 0) {
    // Display an error message if there are associated students
    echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
            <div class="alert alert-danger role="alert" style="color:#000;text-align:center;margin-bottom:40px;font-family:cairo">
                لا يمكن حذف الشعبة لأنها مرتبطة بسجلات طلابية
            </div>
          </div>';
} else {
    // Proceed with the deletion if no students are associated
    $stmt = $con->prepare("DELETE FROM majors WHERE id = :major_id");
    ...
    echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
            <div class="alert alert-success" role="alert" style="color:#FFF;text-align:center;margin-bottom:40px;font-family:cairo">
                تم حذف هذه الشعبة بنجاح
            </div>
          </div>';
}

// Rest of your HTML and page content here
?>
    
                                       <?php
// Fetch majors data and display
$sql = $con->prepare("SELECT majors.*, academic_admins.name FROM majors INNER JOIN academic_admins ON academic_admins.id = majors.academic_admin_id");
$sql->execute();
$rows = $sql->fetchAll();
...
<tr>
    ...
    <td><?php echo $pat['number_of_students']; ?></td>
    <td><?php echo $pat['term']; ?></td>
    <td><?php echo $pat['year']; ?></td>
    <td>
        <a href="majors_edit.php?major_id=<?php echo $pat['id']; ?>" class="btn btn-primary"><i class="fa fa-edit"></i></a>
        <a onclick="return confirm('هل أنت متأكد من عملية الحذف ؟');" href="delete_major.php?major_id=<?php echo $pat['id']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i></a>
    </td>
</tr>
<?php } ?>
