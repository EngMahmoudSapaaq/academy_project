                        ...
                        <div class="page-title-box d-flex align-items-center justify-content-between">
                            <div class="page-title-right">
                                <ol class="breadcrumb m-0">
                                    <li class="breadcrumb-item">ادارة المشرفين الميدانيين</li>
...
if(isset($_POST['edit'])){
    ...
    // التحقق من عدم تكرار البريد الإلكتروني أو الهاتف في جدول training_admins
    $sql = $con->prepare("SELECT * FROM training_admins WHERE (email = ? OR phone = ?) AND id != ?");
    $sql->execute([$email, $phone, $train_id]);
    $countAdmin = $sql->rowCount();

    // التحقق من عدم تكرار اسم المستخدم في جدول accounts
    $sql2 = $con->prepare("SELECT * FROM accounts WHERE username = ? AND id != ?");
    $sql2->execute([$username, $account_id]);
    $countUsername = $sql2->rowCount();

    if($countAdmin == 0 && $countUsername == 0){

        if(!empty($_POST['password']) && !empty($_POST['confirm'])){
            if($_POST['password'] == $_POST['confirm']){
                $stmt = $con->prepare("UPDATE training_admins SET email = ?, phone = ?, training_agency_id = ? WHERE id = ?");
                $stmt->execute([$email, $phone, $term, $train_id]);

                $stmt1 = $con->prepare("UPDATE accounts SET username = ?, password = ? WHERE id = ?");
                $stmt1->execute([$username, $_POST['password'], $account_id]);
                ...
                echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
                          <div class="alert alert-success" role="alert" style="color:#FFF;text-align:center;margin-bottom:40px;font-family:cairo">
                              تم تحديث بيانات المشرف الميداني بنجاح
                          </div>
                      </div>';
            } else {
                echo '<div class="container" dir="rtl" style="margin-top:30px;color:#FFF;margin-bottom:30px;font-family:cairo">
...
?>
                                      ...
                                      <?php
                                        ...
                                        include('../connect.php');
                                        $stmt = $con->prepare("SELECT training_admins.* , accounts.username FROM training_admins INNER JOIN accounts ON accounts.id=training_admins.account_id WHERE training_admins.id='$train_id'");  
                                        $stmt->execute();
                                        $rowsSpec = $stmt->fetch();
                                        $count = $stmt->rowCount();
                                      ...
                                      ?> 
                                        <form method="post" class="form">
                                            <input type="hidden" name="train_id" value="<?php echo $rowsSpec['id']; ?>"/>
