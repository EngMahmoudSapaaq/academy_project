...
if(isset($_POST['add'])){
    ...
    if($password == $confirm){  
        ...
        $checkStmt = $con->prepare("SELECT * FROM training_admins 
                                    JOIN accounts ON training_admins.account_id = accounts.id
                                    WHERE training_admins.email = :email 
                                    OR training_admins.phone = :phone 
        ...
        if($checkStmt->rowCount() > 0){ 
            ...
            $sql = "INSERT INTO accounts (username, password, status, type) VALUES (:username, :password, 'activated', 'trainning_supervisor')";
            ...
            // إدخال البيانات في جدول training_admins
            $sql1 = "INSERT INTO training_admins (name, email, phone, gender, training_agency_id, account_id) VALUES (:name, :email, :phone, :gender, :training_agency_id, :account_id)";
            ...
            echo '
                <div class="container" dir="rtl" style="margin-top:80px;color:#000">
                    <div class="alert alert-success" role="alert" style="color:#000">
                        تم اضافة المشرف التدريبي بنجاح
                    </div>
                </div>';
        }
    } else {
