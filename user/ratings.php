...
//------------------------------------------
$student_scores = mysqli_query($conn, "SELECT * FROM `student_scores` WHERE student_id = '$student_id'") or die('Query failed: ' . mysqli_error($conn));
...
<body id="page-top">
    ...
    <section class="page-section mt-5 d-flex justify-content-center align-items-center" style="min-height: 100vh;" id="services">
        <div class="container px-4 px-lg-5">
            <h2 class="text-center mt-0">نتائج التقييمات</h2>
            <hr class="divider" />
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                <?php
                                $total_score=0;
                                    if (isset($student_scores) && mysqli_num_rows($student_scores) > 0) {
                                        while ($student_score = mysqli_fetch_assoc($student_scores)) {
                                            $total_score+=$student_score['evaluation_score'];
                                            echo ' <tr>
                                    <td>'.$student_score['evaluation_element'].'</td>
                                   
                                    <td>
                                        '.$student_score['evaluation_score'].'
                                    </td>
                                </tr>';
                            ...
                            <tfoot>
                                <tr class="table-light">
                                    <th colspan="1">المجموع النهائي</th>
