...
//------------------------------------------
$student_attends = mysqli_query($conn, "SELECT * FROM `student_attends` WHERE student_id = '$student_id' AND is_attended = '0'") or die('Query failed: ' . mysqli_error($conn));
?>
<!DOCTYPE html>
<html lang="ar" dir="rtl">
    ...
    <body id="page-top">
        ...
        <section class="page-section mt-5 d-flex justify-content-center align-items-center" style="min-height: 100vh;" id="services">
            <div class="container px-4 px-lg-5">
                <h2 class="text-center mt-0">سجل الغياب</h2>
                <hr class="divider" />
                    ...
                    <div class="col-12 text-center">
                        <div class="table-responsive">
                            <table class="table">
                                ...
                                <tbody>
                                    <?php
                                    if (isset($student_attends) && mysqli_num_rows($student_attends) > 0) {
                                        while ($student_attend = mysqli_fetch_assoc($student_attends)) {

                                            echo '<tr>
                                    <td>' . $student_attend['date'] . '</td>
                                    <td><span class="badge bg-danger">غائب</span></td>
                                </tr>';
                                        }
                                    } else {
                                        echo '<tr>
                                    <td>لا يتوفر بيانات في سجل الغياب.</td>
                                    <td></td>
                                </tr>';
                                    }
                                    ?>
                                ...
                                <tfoot>
                                    <tr class="table-light">
                                        <th>مجموع :الغياب</th>
                                        <td colspan="2"><?= mysqli_num_rows($student_attends) ?? 0 ?></td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
